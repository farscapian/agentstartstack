...
**Idempotency / re-running.** The init scripts are convergent -- re-running lands the clone in the same aligned state. When the clone is clean and does not diverge from canonical, align is a plain **fast-forward** (`git merge --ff-only local-sync/main`) that destroys nothing. It falls back to a **hard reset** (`git reset --hard local-sync/main` + `git clean -fd`) only when there is work to discard, and that reset **discards uncommitted work and untracked files** (gitignored files such as `.agentstartstack-bump` survive). It can also discard **committed-but-unlanded work** -- commits made in the clone that are ahead of `local-sync/main` but not yet landed on canonical -- because the reset moves HEAD back to canonical's tip (dropped commits are recoverable only via reflog). The scripts guard both hazards on re-run: they detect a dirty working tree **and** commits ahead of `local-sync/main`, and before discarding either they prompt interactively (showing the ahead commits) or, when non-interactive (`-y` / no tty, e.g. the SessionStart hook), **refuse and exit non-zero** rather than destroy work. A fresh first-run clone is clean and not ahead, so it fast-forwards silently. The safe ordering is therefore: **commit in the clone -> land it (`ass sync`) -> only then re-adopt/align.** To merely pick up a pending bump in an active session, do **not** re-run init -- apply the watch file directly (see [The .agentstartstack-bump watch file](#the-agentstartstack-bump-watch-file)).
...
**Agent obligation:** a pending `.agentstartstack-bump` is a debt to settle *eventually*, not a gate on your own work -- it never blocks a commit, and you are free to keep committing your consumer changes with the flag still present. Settle it at the next convenient point (ideally before you wrap up the session). When you do, **do not just bump the pointer** -- read what changed in the producer and reconcile this consumer's own copy with it:
...
**Reminder (pre-commit hook), not a blocker.** The init scripts install a pre-commit hook in the session clone (under `.git/agentstartstack-hooks/`, with `core.hooksPath` pointed at it) that **prints a reminder while `.agentstartstack-bump` is present but never blocks the commit** -- a pending bump must not stand between a consumer and its own work. The bump is handled *eventually*: you reconcile it and drop the flag on this commit or a later one, whichever is convenient, and the reminder keeps surfacing until you do. The hook then chains to the repo's tracked `.githooks/pre-commit`, so shellcheck and any other checks still run. It lives under `.git/` so it survives `reset --hard` + `git clean -fd`.

The hook is installed by `scripts/install-precommit-guard.sh`, which **both** the init scripts **and** `install-githooks.sh` call. So running either installs the same hook -- they cannot diverge, and re-running `install-githooks.sh` re-asserts it rather than dropping it. Agents do not need to remember to re-run anything.
...
**Init backstop.** `init_*_session.sh` calls `agentstartstack_pending_consumer_actions`: when the submodule pointer is already current but any `CONSUMER-ACTION` in `(watermark..HEAD]` remains unperformed, session align prints the pending range and **drops the `.agentstartstack-bump` watch file** so the pre-commit reminder keeps surfacing until the actions are performed and the watermark recorded. This catches consumers that advanced the pointer without reconciling (for example a pre-action-aware blind auto-bump) and keeps the obligation visible on every commit rather than letting it disappear silently. It does not block commits -- the consumer keeps working and reconciles the bump eventually.
...
**Init backstop (surfaces action-bearing deferrals).** Independently of any watch file, `init_*_session.sh` checks at align time whether the `.agentstartstack` submodule is behind its remote (`agentstartstack_pending_reconcile`). If so it prints the pending `OLD..NEW` range and the read-and-reconcile commands. When that pending delta **carries a `CONSUMER-ACTION:`** (`agentstartstack_range_has_consumer_action`), the backstop **drops the `.agentstartstack-bump` watch file into this clone**, so the pre-commit reminder keeps resurfacing on every commit until the agent reconciles and removes it -- the same persistent flag an in-flight clone gets, now extended to the no-in-flight case where `ass publish` had no clone to flag at publish time. An action-free behind-remote delta stays a soft warning (no flag), since it needs no host action. The flag never blocks a commit; it keeps a deferred action-bearing bump from disappearing silently, so the consumer performs every `CONSUMER-ACTION` in the delta eventually rather than never.
