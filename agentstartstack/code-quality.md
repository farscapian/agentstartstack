...
- `usage()` extracts help from header comment block via `sed` when applicable
- Secrets only from `.env` files, never CLI flags
- `log()` / `die()` / `info()` helpers for consistent output
- `trap cleanup EXIT` for mount/unmount or temp file cleanup