...
These repos already have inline `agentstartstack/`. Migration path:

1. Add `agentstartstack` submodule
2. Run `add-to-project.sh` (creates wrappers; does not delete existing `agentstartstack/`)
3. Trim duplicated generic sections from project `agentstartstack/workflow.md` and `nut.md` over time; link to `.agentstartstack/agentstartstack/` instead
