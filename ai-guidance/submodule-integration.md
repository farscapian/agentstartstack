...
│   ├── init_claude_session.sh    # thin wrapper -> agentstartstack
│   ├── install-githooks.sh       # thin wrapper -> agentstartstack
│   └── shellcheck-staged.sh      # thin wrapper -> agentstartstack
└── .githooks/pre-commit          # thin wrapper -> agentstartstack
```
...
$EDITOR CLAUDE.md
$EDITOR ai-guidance/README.md

git add agentstartstack .agentstartstack.env CLAUDE.md ai-guidance scripts .githooks
...
2. Link generic workflow: `agentstartstack/ai-guidance/workflow.md`, `nut.md`
3. Index project-specific files under `ai-guidance/`
...
These repos already have inline `ai-guidance/`. Migration path:

1. Add `agentstartstack` submodule
2. Run `add-to-project.sh` (creates wrappers; does not delete existing `ai-guidance/`)
3. Trim duplicated generic sections from project `ai-guidance/workflow.md` and `nut.md` over time; link to `agentstartstack/ai-guidance/` instead
